...
import { Generators } from '../src/lib/3_voxelGenerators.js';
...
function getLocalPresetFromPrompt(value: string): 'Fox' | 'Tiger' | null {
  ...
  if (normalized.includes('fox') || normalized.includes('鐙愮嫺') || normalized.includes('灏忕嫄鐙?)) {
    return 'Fox';
  }
  if (normalized.includes('tiger') || normalized.includes('鑰佽檸') || normalized.includes('灏忚�佽檸')) {
    return 'Tiger';
  }
  return null;
}
