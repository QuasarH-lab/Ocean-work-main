...
export default function Generator() {
  ...
  function persistBuild(build: {
    ...
    const savedModel: SavedModel = {
      ...
      mode: build.mode,
      createdAt: Date.now(),
      baseModel: build.baseModel || undefined,
      data: build.data,
      bricks: voxelsToBricks(build.data),
    };

    if (build.mode === 'morph') {
      setCustomRebuilds((prev) => [savedModel, ...prev.filter((item) => item.id !== id)]);
    } else {
      setCustomBuilds((prev) => [savedModel, ...prev.filter((item) => item.id !== id)]);
    }
  }
  ...
  async function handleGenerate(mode: GenerateMode) {
    ...
    try {
      ...
      const shouldUseLocalFox =
        mode === 'create' &&
        (normalizedPrompt.includes('fox') || normalizedPrompt.includes('鐙愮嫺') || normalizedPrompt.includes('灏忕嫄鐙?));
